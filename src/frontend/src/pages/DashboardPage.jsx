...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      <section>
        ...
        <div className="rounded border border-app-border bg-app-panel">
          <div className="grid grid-cols-2 md:grid-cols-5">
            ...
            <StripCell label="High + critical" value={int(d.critical_high_priority?.count)} sub={`${int(d.critical_high_priority?.critical)} critical · ${int(d.critical_high_priority?.high)} high`} tone={d.critical_high_priority?.count > 0 ? "text-high" : ""} />
      ...
      {/* ANALYTICAL ROW — RISK PROFILE (65) | SYSTEM HEALTH (35) */}
      <div className="mt-4 grid gap-4 lg:grid-cols-10">
        <div className="lg:col-span-6">
          <div className={PANEL}>
            <div className={PANEL_HEAD}>
              <p className={SECTION_TITLE}>Asset risk profile</p>
              <span className={MUTED}>model-estimated failure distribution</span>
            </div>
            <div className="p-4">
              <RiskProfileBars distribution={d.risk} />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className={PANEL}>
            ...
            <div className="divide-y divide-app-borderDim text-xs">
              ...
              <HealthRow label="Critical assets" value={int(d.priority?.CRITICAL)} tone={d.priority?.CRITICAL > 0 ? "text-critical" : ""} />
              <HealthRow label="High priority assets" value={int(d.priority?.HIGH)} tone={d.priority?.HIGH > 0 ? "text-high" : ""} />
...
function RiskProfileBars({ distribution }) {
